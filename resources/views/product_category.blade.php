                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <h4><span class="text-body">Product Category</span></h4>
                        ...
                        <div class="item-list activity " id="activity">
                            <table id="itemTable" class="table table-bordered table-hover verticle-middle">
                                <tbody>
                                    ...
                                    @if($list_product_category->count() > 0)
                                    @foreach($list_product_category as $value)
                                    <tr class="category-rowid" data-id="{{ $value->id }}"
                                        data-name="{{ $value->product_category_name ? $value->product_category_name : null  }}">
                                        <td>
                                            {{-- <a id="get_data">{{$value->product_category_name}}</a> --}}
                                            <a href="#" id="item-link" data-id="{{ $value->id ? $value->id : null  }}"
                                                data-name="{{ $value->product_category_name ? $value->product_category_name : null  }}">
                                                {{ $value->product_category_name ? $value->product_category_name : null}}</a>
                                        </td>
                                    </tr>
                                    @endforeach
                                    @endif
...
@include('layouts.product_category_modal')
@include('layouts.product_category_ajax')
